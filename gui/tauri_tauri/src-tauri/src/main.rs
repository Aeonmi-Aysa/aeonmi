...
use serde_json::json;
use quantum_ide::setup_tauri_app;
...
fn main() {
    ...
    // Setup quantum IDE application
    let mut app = setup_tauri_app();
    
    // Add legacy commands for compatibility
    app = app.on_window_event(|_w, e| { 
        if let tauri::WindowEvent::CloseRequested { .. } = e { 
            let mut reg = PTY_REGISTRY.lock().unwrap(); 
            for (_id, entry) in reg.drain() { 
                let _ = entry.child.lock().unwrap().kill(); 
            } 
        } 
    });
    
    // Generate context and run
