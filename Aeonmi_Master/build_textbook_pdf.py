...
# ---------------------------------------------------------------------------
# PDF merge helper (no external dependencies — uses ReportLab's reader)
# ---------------------------------------------------------------------------

def _merge_pdfs(part_paths: list, output_path: Path):
    """Merge multiple PDF files into one using pypdf (pip install pypdf)."""
    try:
        ...
        try:
            ...
            print("  Installing pypdf for PDF merge step...")
...
def _build_part(fpath: Path, styles: dict, part_pdf: Path,
                include_cover: bool, toc_entries_out: list):
    """Build one input file into a standalone PDF chunk."""
    import gc

    tokens = parse_file(fpath)
    toc_local = []
    elements = tokens_to_flowables(tokens, styles, toc_local)
    toc_entries_out.extend(toc_local)

    all_el = []
    if include_cover:
        all_el.extend(build_cover(styles))
        all_el.extend(build_toc(toc_local, styles))
    all_el.extend(elements)
    ...
    doc.build(
        all_el,
        onFirstPage=_cover_template if include_cover else _page_template,
...
def build_pdf():
    ...
    # Build each input file as a separate PDF part to avoid canvas MemoryError
    print("Parsing and building parts...")
    part_pdfs = []
    all_toc = []
    tmp_dir = Path(tempfile.mkdtemp(prefix="aeonmi_pdf_"))

    for i, fpath in enumerate(INPUT_FILES):
        part_path = tmp_dir / f"part_{i}.pdf"
        label = fpath.name if fpath.exists() else f"{fpath.name} (placeholder)"
        print(f"  Building {label} ...", flush=True)
        try:
            count = _build_part(fpath, styles, part_path, include_cover=(i == 0), toc_entries_out=all_toc)
            print(f"    {count} flowables -> {part_path.name}")
            part_pdfs.append(part_path)
        except Exception as e:
            print(f"    ERROR building {fpath.name}: {e}")
            raise

    print()
    print(f"  Total TOC entries: {len(all_toc)}")
    print(f"  Parts built: {len(part_pdfs)}")
    print()

    if len(part_pdfs) == 1:
        import shutil
        shutil.copy(str(part_pdfs[0]), str(OUTPUT_PDF))
    else:
        print("Merging parts into final PDF...")
        _merge_pdfs(part_pdfs, OUTPUT_PDF)
