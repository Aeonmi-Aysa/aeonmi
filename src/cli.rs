...
pub enum Command {
    ...
    /// Run a .qube quantum circuit file
    ///
    /// Examples:
    ...
    /// Mint an Aeonmi .ai artifact as NFT metadata (Solana-compatible)
    ///
    /// Examples:
    ///   aeonmi mint output.ai
    ///   aeonmi mint output.ai --personality quantum-titan --anchor
    Mint {
        /// Source .ai file to mint
        #[arg(value_name = "FILE")]
        file: std::path::PathBuf,
        /// Personality tag embedded in attributes
        #[arg(long = "personality", default_value = "quantum-titan")]
        personality: String,
        /// Also emit the Anchor Rust stub
        #[arg(long = "anchor", action = ArgAction::SetTrue)]
        anchor: bool,
        /// Custom glyph seed (hex string)
        #[arg(long = "glyph-seed", value_name = "HEX")]
        glyph_seed: Option<String>,
        /// Write metadata JSON to file instead of stdout
        #[arg(long = "out", value_name = "FILE")]
        out: Option<std::path::PathBuf>,
    },

    /// Run Mother AI embryo loop (interactive REPL or single file)
    ///
    /// Examples:
    ///   aeonmi mother            (interactive REPL)
    ///   aeonmi mother --file script.ai
    ///   aeonmi mother --creator Warren --verbose
    Mother {
        /// Run a .ai script file through Mother (non-interactive)
        #[arg(long = "file", value_name = "FILE")]
        file: Option<std::path::PathBuf>,
        /// Creator identifier for quantum bond
        #[arg(long = "creator", default_value = "Warren")]
        creator: String,
        /// Print verbose stats after each interaction
...
#[derive(Subcommand, Debug, Clone)]
pub enum QubeAction {
    /// Run a .qube file: parse → execute → print circuit diagram
    Run {
        #[arg(value_name = "FILE")]
        file: std::path::PathBuf,
        /// Print the text-mode circuit diagram after execution
        #[arg(long = "diagram", action = ArgAction::SetTrue)]
        diagram: bool,
    },
    /// Validate a .qube file (parse only, no execution)
    Check {
        #[arg(value_name = "FILE")]
        file: std::path::PathBuf,
    },
