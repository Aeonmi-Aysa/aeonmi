...
impl CircuitVisualizer {
    ...
    pub fn to_ascii(&self, circuit: &QuantumCircuitBuilder) -> String {
        ...
        output.push_str(&format!("Qubits: {}, Gates: {}, Depth: {}\n", 
                               circuit.qubit_count(), 
                               circuit.gate_count(), 
                               circuit.depth()));
    ...
    fn add_gate_to_diagram(&self, lines: &mut Vec<String>, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) {
    ...
    fn get_gate_qubit_indices(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Vec<usize> {
        gate.qubits.iter()
    ...
    fn add_single_gate_to_diagram(&self, lines: &mut Vec<String>, qubit_indices: &[usize], symbol: &str) {
        ...
        for (i, line) in lines.iter_mut().enumerate() {
            if qubit_indices.contains(&i) {
                ...
                line.push_str(&format!("─{}─", self.horizontal_char().repeat(max_symbol_len)));
            }
        }
    }
    ...
    fn add_controlled_gate_to_diagram(&self, lines: &mut Vec<String>, qubit_indices: &[usize], gate_symbol: &str) {
        if qubit_indices.len() != 2 { return; }
    ...
    pub fn to_latex(&self, circuit: &QuantumCircuitBuilder) -> String {
        ...
        for (i, qubit) in circuit.qubits.iter().enumerate() {
            latex.push_str(&format!("\\node[anchor=east] at (0,{}) {{$|{}\\rangle$}};\n", 
                                   -(i as f64), qubit.name));
            latex.push_str(&format!("\\draw (0.5,{}) -- (10,{});\n", 
                                   -(i as f64), -(i as f64)));
        }
    ...
    fn gate_to_latex(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder, x_pos: f64) -> String {
        let mut latex = String::new();
        let qubit_indices = self.get_gate_qubit_indices(gate, circuit);
        ...
        match &gate.gate_type {
            QuantumGateType::Hadamard => {
                if let Some(&qubit) = qubit_indices.first() {
                    latex.push_str(&format!("\\gate[wires=1]{{H}} ({},{});\n", x_pos, -(qubit as f64)));
                }
            },
            QuantumGateType::CNOT => {
                if qubit_indices.len() == 2 {
                    let control = qubit_indices[0];
                    let target = qubit_indices[1];
                    latex.push_str(&format!("\\ctrl{} ({},{});\n", target as i32 - control as i32, x_pos, -(control as f64)));
            ...
            _ => {
                ...
                if let Some(&qubit) = qubit_indices.first() {
                    ...
                    latex.push_str(&format!("\\gate[wires=1]{{{}}} ({},{});\n", symbol, x_pos, -(qubit as f64)));
                }
            }
        }
