...
impl QuantumCircuitCompiler {
    ...
    pub fn compile(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        match self.options.target {
            ...
            _ => Err(CompilationError::UnsupportedTarget(format!("{:?}", self.options.target))),
        }
    }
    
    /// Compile to OpenQASM 2.0
    fn compile_to_qasm2(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
    ...
    fn compile_to_qasm3(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let mut qasm = String::new();
    ...
    fn compile_to_qiskit(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        python.push_str(&format!("qreg = QuantumRegister({}, 'q')\n", circuit.qubit_count()));
        
        if self.options.include_measurements {
            python.push_str(&format!("creg = ClassicalRegister({}, 'c')\n", circuit.qubit_count()));
        ...
        for (name, value) in &circuit.parameters {
            python.push_str(&format!("{} = Parameter('{}')  # value: {}\n", name, name, value));
        }
    ...
    fn compile_to_cirq(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        // Qubit creation
        python.push_str(&format!("# AEONMI Generated Circuit: {}\n", circuit.name));
        python.push_str(&format!("qubits = [cirq.NamedQubit('q{{}}').format(i) for i in range({})]\n", circuit.qubit_count()));
    ...
    fn compile_to_qsharp(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        qsharp.push_str(&format!("    operation {}() : Result[] {{\n", circuit.name.replace(" ", "")));
        qsharp.push_str(&format!("        use qubits = Qubit[{}];\n", circuit.qubit_count()));
    ...
    fn compile_to_javascript(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        js.push_str("const { QuantumSimulator, QuantumGates } = require('@aeonmi/quantum-runtime');\n\n");
        js.push_str(&format!("const circuit = new QuantumSimulator({});\n", circuit.qubit_count()));
    ...
    fn compile_to_aeonmi(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        for gate in &circuit.gates {
            aeonmi_code.push_str(&format!("    {},\n", self.gate_to_aeonmi(gate, circuit)?));
        }
    ...
    fn compile_to_simulator(&self, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
    ...
    fn gate_to_qasm2(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        ...
        match &gate.gate_type {
            ...
            QuantumGateType::CNOT => Ok(format!("cx q[{}], q[{}];", qubit_indices[0], qubit_indices[1])),
            QuantumGateType::CZ => Ok(format!("cz q[{}], q[{}];", qubit_indices[0], qubit_indices[1])),
            QuantumGateType::RotationZ(angle) => Ok(format!("rz({}) q[{}];", angle, qubit_indices[0])),
            QuantumGateType::Measure => {
                if self.options.include_measurements {
                    Ok(format!("measure q[{}] -> c[{}];", qubit_indices[0], qubit_indices[0]))
                } else {
                    Ok(String::new())
                }
            },
            _ => Err(CompilationError::UnsupportedGate(format!("{:?}", gate.gate_type))),
        }
    }
    
    fn gate_to_qasm3(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let qubit_indices = self.get_qubit_indices(gate, circuit)?;
        ...
        match &gate.gate_type {
            ...
            QuantumGateType::Measure => {
                if self.options.include_measurements {
                    Ok(format!("c[{}] = measure q[{}];", qubit_indices[0], qubit_indices[0]))
                } else {
                    Ok(String::new())
                }
            },
    ...
    fn gate_to_qiskit(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let qubit_indices = self.get_qubit_indices(gate, circuit)?;
        ...
        match &gate.gate_type {
            ...
            QuantumGateType::CNOT => Ok(format!("circuit.cx(qreg[{}], qreg[{}])", qubit_indices[0], qubit_indices[1])),
            QuantumGateType::RotationZ(angle) => Ok(format!("circuit.rz({}, qreg[{}])", angle, qubit_indices[0])),
            QuantumGateType::Measure => {
                if self.options.include_measurements {
                    Ok(format!("circuit.measure(qreg[{}], creg[{}])", qubit_indices[0], qubit_indices[0]))
                } else {
                    Ok(String::new())
                }
            },
    ...
    fn gate_to_cirq(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let qubit_indices = self.get_qubit_indices(gate, circuit)?;
        
        match &gate.gate_type {
            QuantumGateType::Hadamard => Ok(format!("circuit.append(cirq.H(qubits[{}]))", qubit_indices[0])),
            QuantumGateType::PauliX => Ok(format!("circuit.append(cirq.X(qubits[{}]))", qubit_indices[0])),
            QuantumGateType::CNOT => Ok(format!("circuit.append(cirq.CNOT(qubits[{}], qubits[{}]))", qubit_indices[0], qubit_indices[1])),
            _ => Err(CompilationError::UnsupportedGate(format!("{:?}", gate.gate_type))),
        }
    }
    
    fn gate_to_qsharp(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let qubit_indices = self.get_qubit_indices(gate, circuit)?;
        
        match &gate.gate_type {
            QuantumGateType::Hadamard => Ok(format!("H(qubits[{}])", qubit_indices[0])),
            QuantumGateType::PauliX => Ok(format!("X(qubits[{}])", qubit_indices[0])),
            QuantumGateType::CNOT => Ok(format!("CNOT(qubits[{}], qubits[{}])", qubit_indices[0], qubit_indices[1])),
            _ => Err(CompilationError::UnsupportedGate(format!("{:?}", gate.gate_type))),
        }
    }
    
    fn gate_to_javascript(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<String, CompilationError> {
        let qubit_indices = self.get_qubit_indices(gate, circuit)?;
        
        match &gate.gate_type {
            QuantumGateType::Hadamard => Ok(format!("circuit.h({});", qubit_indices[0])),
            QuantumGateType::PauliX => Ok(format!("circuit.x({});", qubit_indices[0])),
            QuantumGateType::CNOT => Ok(format!("circuit.cnot({}, {});", qubit_indices[0], qubit_indices[1])),
    ...
    fn get_qubit_indices(&self, gate: &QuantumGate, circuit: &QuantumCircuitBuilder) -> Result<Vec<usize>, CompilationError> {
        gate.qubits.iter()
            .map(|qubit_id| {
                circuit.qubits.iter()
...
impl std::fmt::Display for CompilationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CompilationError::UnsupportedTarget(target) => write!(f, "Unsupported compilation target: {}", target),
            ...
            CompilationError::HardwareConstraintViolation(msg) => write!(f, "Hardware constraint violation: {}", msg),
        }
    }
}
