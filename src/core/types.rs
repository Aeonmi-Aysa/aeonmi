...
impl TypeContext {
    ...
    fn declare(&mut self, name: &str, ty: TypeKind) {
    ...
    fn update_if_unknown(&mut self, name: &str, ty: &TypeKind) {
        if *ty == TypeKind::Unknown || *ty == TypeKind::Void {
            return;
        }
        for s in self.scopes.iter().rev() {
            if let Some(slot) = s.get(name) {
                if *slot != TypeKind::Unknown {
                    return;
                }
            }
        }
    ...
    pub fn infer_program(&mut self, ast: &ASTNode) {
        self.visit(ast);
    }
    ...
    fn visit(&mut self, node: &ASTNode) -> TypeKind {
        match node {
            ...
            ASTNode::Function {
                ...
                // Predeclare params
                for p in params {
                    self.declare(&p.name, TypeKind::Unknown);
                }
                let mut ret_type: TypeKind = TypeKind::Void;
                for it in body {
                    if let ASTNode::Return(expr) = it {
                        ret_type = self.visit(expr);
                    } else {
                        self.visit(it);
                    }
                }
                let param_types: Vec<TypeKind> =
                    params.iter().map(|p| self.lookup(&p.name)).collect();
                self.functions
            ...
            ASTNode::VariableDecl {
                ...
                if t == TypeKind::Void {
                    self.diags.push(TypeDiagnostic {
                        message: format!("Variable '{name}' initialized with void"),
                        line: *line,
                        column: *column,
                    });
                }
            ...
            ASTNode::If {
                ...
                if ct != TypeKind::Boolean && ct != TypeKind::Unknown {
                    self.diags.push(TypeDiagnostic {
                        message: "If condition not boolean".into(),
                        line: 0,
                        column: 0,
                    });
                }
            ...
            ASTNode::While { condition, body } => {
                ...
                if ct != TypeKind::Boolean && ct != TypeKind::Unknown {
                    self.diags.push(TypeDiagnostic {
                        message: "While condition not boolean".into(),
                        line: 0,
                        column: 0,
                    });
                }
            ...
            ASTNode::For {
                ...
                if let Some(c) = condition {
                    ...
                    if ct != TypeKind::Boolean && ct != TypeKind::Unknown {
                        self.diags.push(TypeDiagnostic {
                            message: "For condition not boolean".into(),
                            line: 0,
                            column: 0,
                        });
                    }
            ...
            ASTNode::BinaryExpr { op, left, right } => {
                ...
                let result = match op {
                    Plus | Minus | Star | Slash => {
                        if lt == TypeKind::Unknown && rt == TypeKind::Number {
                            return TypeKind::Number;
                        }
                        if rt == TypeKind::Unknown && lt == TypeKind::Number {
                            return TypeKind::Number;
                        }
                        if lt != TypeKind::Number || rt != TypeKind::Number {
                            self.diags.push(TypeDiagnostic {
                                message: "Arithmetic on non-number".into(),
                                line: 0,
                                column: 0,
                            });
                            TypeKind::Unknown
                    ...
                    DoubleEquals | NotEquals => {
                        if lt != rt && lt != TypeKind::Unknown && rt != TypeKind::Unknown {
                            self.diags.push(TypeDiagnostic {
                                message: "Equality between different types".into(),
                                line: 0,
                                column: 0,
                            });
                        }
                    ...
                    GreaterThan | GreaterEqual | LessThan | LessEqual => {
                        if lt != TypeKind::Number || rt != TypeKind::Number {
                            self.diags.push(TypeDiagnostic {
                                message: "Comparison on non-number".into(),
                                line: 0,
                                column: 0,
                            });
                        }
            ...
            ASTNode::Call { callee, args } => {
                ...
                let (fname, f_info) = match &**callee {
                    ASTNode::Identifier(n) | ASTNode::IdentifierSpanned { name: n, .. } => {
                        let info = self.functions.get(n).cloned();
                        (n.clone(), info)
                    }
                    _ => (String::new(), None),
                };
                ...
                if let Some((param_tys, ret_ty)) = f_info {
                    ...
                    if param_tys.len() != args.len() {
                        self.diags.push(TypeDiagnostic {
                            message: format!("Call arity mismatch for {fname}"),
                            line: 0,
                            column: 0,
                        });
                    }
