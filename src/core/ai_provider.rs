...
impl AiProvider for FauxProvider {
    ...
    async fn chat_stream(&self, prompt: &str, sink: &mut dyn ChunkSink) -> Result<(), String> {
        ...
        for part in text.as_bytes().chunks(12) {
            sink.emit(ChatChunk {
                content: String::from_utf8_lossy(part).to_string(),
                done: false,
            })
            .await;
        }
...
impl ProviderRegistry {
    pub fn new() -> Self {
        let mut reg = Self {
            providers: vec![Arc::new(FauxProvider)],
            active: "faux".into(),
        };
        reg.load_active();
        reg
    ...
    fn prefs_path() -> Option<std::path::PathBuf> {
        dirs_next::config_dir().map(|p| p.join("aeonmi").join("ai_prefs.json"))
    }
    ...
    fn save_active(&self) -> std::io::Result<()> {
        if let Some(path) = Self::prefs_path() {
            if let Some(dir) = path.parent() {
                std::fs::create_dir_all(dir)?;
            }
            let v = serde_json::json!({"active": self.active});
            std::fs::write(
                path,
