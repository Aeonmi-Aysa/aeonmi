...
fn collect(node: &ASTNode, out: &mut Vec<CodeAction>) {
    match node {
        ...
        ASTNode::Function {
            ...
            if body.len() > 5 {
                out.push(CodeAction {
                    title: format!("Extract part of '{name}' into function"),
                    kind: "extractFunction".into(),
                    line: *line,
                    column: *column,
                });
            }
        ...
        ASTNode::VariableDecl {
            ...
            out.push(CodeAction {
                title: format!("Rename variable '{name}'"),
                kind: "rename".into(),
                line: *line,
                column: *column,
            });
        ...
        ASTNode::Assignment { line, column, .. } => {
            out.push(CodeAction {
                title: "Introduce variable".into(),
                kind: "introduceVariable".into(),
                line: *line,
                column: *column,
            });
        }
...
fn add_missing_let_actions(ast: &ASTNode, out: &mut Vec<CodeAction>) {
    ...
    fn walk(
        ...
        match node {
            ...
            ASTNode::Program(items) | ASTNode::Block(items) => {
                for it in items {
                    walk(it, declared, out);
                }
            }
            ASTNode::Function { body, .. } => {
                for it in body {
                    walk(it, &mut declared.clone(), out);
                }
            }
            ASTNode::If {
                then_branch,
                ...
                walk(then_branch, &mut declared.clone(), out);
...
fn add_inline_variable_actions(ast: &ASTNode, out: &mut Vec<CodeAction>) {
    ...
    for (name, info) in map.iter() {
        if info.uses == 1 {
            if let Some((l, c)) = info.decl {
                out.push(CodeAction {
                    title: format!("Inline variable '{name}'"),
                    kind: "inlineVariable".into(),
                    line: l,
                    column: c,
                });
            }
