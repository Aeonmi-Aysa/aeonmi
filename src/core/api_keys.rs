...
fn storage_path() -> PathBuf {
    if let Ok(base) = std::env::var("AEONMI_CONFIG_DIR") {
        return PathBuf::from(base).join("keys.json");
    }
    config_dir()
...
pub fn get_api_key(provider: &str) -> Option<String> {
    ...
    let raw = data.get(provider)?;
    // Backwards compatibility: either plain base64 (legacy) or JSON object
    let (b64, _ver) = if raw.trim_start().starts_with('{') {
        if let Ok(val) = serde_json::from_str::<serde_json::Value>(raw) {
            if let Some(nc) = val.get("nonce_ct_b64").and_then(|v| v.as_str()) {
                (
                    nc.to_string(),
                    val.get("v").and_then(|v| v.as_u64()).unwrap_or(0) as u32,
                )
            } else {
                ("".to_string(), 0)
            }
...
pub fn rotate_all_keys() -> Result<RotationReport, String> {
    ...
    for prov in providers.iter() {
        match get_api_key(prov) {
            Some(plain) => {
                if let Err(e) = set_api_key(prov, &plain) {
            ...
            None => {
                errors.push((prov.clone(), "decrypt_failed".to_string()));
            }
        }
    }
    Ok(RotationReport {
        attempted: providers.len(),
        rotated,
        errors,
    })
}
