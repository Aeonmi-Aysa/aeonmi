...
impl HardwareManager {
    pub fn new() -> Self {
        ...
        devices.insert("ibmq_qasm_simulator".to_string(), QuantumDevice {
            name: "IBM QASM Simulator".to_string(),
            provider: QuantumProvider::IBMQuantum,
            qubits: 32,
            connectivity: (0..32).map(|i| (i, (i + 1) % 32)).collect(),
            gate_set: vec!["x".to_string(), "y".to_string(), "z".to_string(), "h".to_string(), "cx".to_string(), "rz".to_string()],
            is_available: true,
            queue_length: 0,
            error_rates: [("single_qubit".to_string(), 0.001), ("two_qubit".to_string(), 0.01)].iter().cloned().collect(),
        });
        
        devices.insert("ibm_brisbane".to_string(), QuantumDevice {
            name: "IBM Brisbane".to_string(),
            provider: QuantumProvider::IBMQuantum,
            qubits: 127,
            connectivity: vec![(0, 1), (1, 2), (2, 3), (3, 4)], // Simplified topology
            gate_set: vec!["x".to_string(), "sx".to_string(), "rz".to_string(), "cx".to_string()],
            is_available: true,
            queue_length: 45,
            error_rates: [("single_qubit".to_string(), 0.0005), ("two_qubit".to_string(), 0.007)].iter().cloned().collect(),
        });
        
        // Add IonQ devices
        devices.insert("ionq_harmony".to_string(), QuantumDevice {
            name: "IonQ Harmony".to_string(),
            provider: QuantumProvider::IonQ,
            qubits: 11,
            connectivity: (0..11).flat_map(|i| (0..11).map(move |j| (i, j))).filter(|(i, j)| i != j).collect(),
            gate_set: vec!["x".to_string(), "y".to_string(), "z".to_string(), "rx".to_string(), "ry".to_string(), "rz".to_string(), "xx".to_string()],
            is_available: true,
            queue_length: 12,
            error_rates: [("single_qubit".to_string(), 0.0001), ("two_qubit".to_string(), 0.002)].iter().cloned().collect(),
        });
        
        // Add local simulator
        devices.insert("aeonmi_simulator".to_string(), QuantumDevice {
            ...
            gate_set: vec!["x".to_string(), "y".to_string(), "z".to_string(), "h".to_string(), "s".to_string(), "t".to_string(), "cx".to_string(), "cz".to_string(), "ccx".to_string()],
            is_available: true,
            queue_length: 0,
    ...
    pub fn submit_job(&mut self, device_name: &str, circuit: QuantumCircuit, shots: usize) -> Result<String, String> {
        ...
        if circuit.qubits > device.qubits {
            return Err(format!("Circuit requires {} qubits, but device only has {}", circuit.qubits, device.qubits));
        }
        
        // Check if all gates are supported
        for gate in &circuit.gates {
            if !device.gate_set.contains(&gate.gate_type) {
                return Err(format!("Gate '{}' not supported on device '{}'", gate.gate_type, device_name));
            }
        }
    ...
    fn execute_simulator_job(&mut self, job_id: &str) -> Result<(), String> {
        ...
        for _ in 0..job.shots {
            ...
            for _ in 0..job.circuit.measurements.len() {
                outcome.push(if rand::random::<f64>() < 0.5 { '0' } else { '1' });
            }
    ...
    fn submit_to_provider(&mut self, job_id: &str) -> Result<(), String> {
        ...
        let device = self.devices.get(&job.device)
    ...
    pub fn get_available_devices_for_provider(&self, provider: &QuantumProvider) -> Vec<&QuantumDevice> {
        self.devices.values()
...
mod tests {
    ...
    fn test_job_submission() {
        ...
        let job_id = manager.submit_job("aeonmi_simulator", circuit, 1000).unwrap();
