        ...
        match op {
            ...
            LoadLocal(i) => {
                if let Some(frame) = self.frames.last() {
                    let v = frame.locals.get(i as usize).cloned().unwrap_or(Value::Null);
                    self.stack.push(v);
                }
            }
            StoreLocal(i) => {
            ...
            And => logical(self, true),
            Or => logical(self, false),
            Pop => {
                self.stack.pop();
            }
            Return => {
...
fn logical(vm: &mut VM, is_and: bool) {
    if let (Some(r), Some(l)) = (vm.stack.pop(), vm.stack.pop()) {
        let lb = matches!(l, Value::Bool(true));
        let rb = matches!(r, Value::Bool(true));
        let res = if is_and { lb && rb } else { lb || rb };
        vm.stack.push(Value::Bool(res));
    }
}
