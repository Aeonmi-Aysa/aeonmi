//! Core module tree for Aeonmi compiler/runtime.
//! Only declare modules that exist in the src/core/ directory.

pub mod ai_emitter;
pub mod ai_provider;
pub mod ast;
pub mod circuit_builder; // Quantum Circuit Builder DSL
pub mod circuit_visualization; // Circuit visualization and ASCII art
pub mod circuit_compiler; // Quantum circuit compilation pipeline
pub mod code_generator;
pub mod code_actions;
pub mod compiler;
pub mod diagnostics;
pub mod error;
pub mod formatter;
pub mod ir;
pub mod lexer;
pub mod lowering;
pub mod incremental;
pub mod parser;
pub mod qpoly;
pub mod quantum_extract;
pub mod quantum_simulator; // Quantum simulation capabilities
pub mod quantum_algorithms; // Standard quantum algorithms library
pub mod hardware_integration; // Real quantum hardware integration
pub mod artifact_cache;
pub mod api_keys;
pub mod semantic_analyzer;
pub mod symbols;
pub mod scope_map;
pub mod types;
pub mod titan;
pub mod token;
pub use token::TokenKind; // Re-export only TokenKind; Token not needed externally currently
#[macro_use]
pub mod debug; // gated debug logging (AEONMI_DEBUG=1) provides debug_log! macro
pub mod vm;

// Phase 2 — Quantum Bridge Integration (ported from quantum_llama_bridge, Llama stripped)
pub mod quantum_neural_network; // QNN layers, entanglement strategies, specialized algorithms
pub mod mother_core;            // MotherQuantumCore, LanguageEvolution, EmotionalCore, Attention
pub mod blockchain;             // SHA256 hash chain, transactions, tamper detection
pub mod mint;                   // NFT metadata generator, Anchor stub, Solana-ready JSON

#[cfg(feature = "bytecode")]
pub mod bytecode;
#[cfg(feature = "bytecode")]
pub mod vm_bytecode;

#[cfg(feature = "quantum")]
pub mod quantum_ir;
