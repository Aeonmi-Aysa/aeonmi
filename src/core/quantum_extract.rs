...
pub fn extract_circuit(ast: &ASTNode) -> QuantumCircuit {
    ...
    QuantumCircuit {
        gates,
        qubit_count: qubit_names.len(),
    }
}

#[allow(dead_code)]
pub fn circuit_to_json(c: &QuantumCircuit) -> String {
...
fn walk(node: &ASTNode, gates: &mut Vec<QuantumGate>, qubits: &mut Vec<String>) {
    match node {
        ASTNode::Program(items) | ASTNode::Block(items) => {
            for it in items {
                walk(it, gates, qubits);
            }
        }
        ASTNode::QuantumOp { op, qubits: qs } => {
            let qn: Vec<String> = qs
                .iter()
        ...
        ASTNode::BinaryExpr { left, right, .. } => {
            walk(left, gates, qubits);
            walk(right, gates, qubits);
        }
        ASTNode::UnaryExpr { expr, .. } => walk(expr, gates, qubits),
        ASTNode::Call { callee, args } => {
            walk(callee, gates, qubits);
            for a in args {
                walk(a, gates, qubits);
            }
        }
