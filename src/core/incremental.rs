...
#[derive(Debug, Clone, Default)]
#[allow(dead_code)]
pub struct TopLevelDiagCache {
    pub per_node: Vec<Vec<crate::core::semantic_analyzer::SemanticDiagnostic>>,
}
#[allow(dead_code)]
pub static DIAG_CACHE: Lazy<Mutex<TopLevelDiagCache>> =
    Lazy::new(|| Mutex::new(TopLevelDiagCache::default()));
pub static LAST_REPLACED_INDEX: Lazy<Mutex<Option<usize>>> = Lazy::new(|| Mutex::new(None));
#[derive(Debug, Clone, Default)]
#[allow(dead_code)]
pub struct TopLevelTypeDiagCache {
    pub per_node: Vec<Vec<crate::core::types::TypeDiagnostic>>,
}
#[allow(dead_code)]
pub static TYPE_DIAG_CACHE: Lazy<Mutex<TopLevelTypeDiagCache>> =
    Lazy::new(|| Mutex::new(TopLevelTypeDiagCache::default()));
...
pub static CALL_GRAPH_METRICS: Lazy<Mutex<CallGraphMetrics>> =
    Lazy::new(|| Mutex::new(CallGraphMetrics::default()));

// Variable dependency tracking: which functions read or write which top-level variables.
#[derive(Debug, Default, Clone)]
pub struct VarDeps {
    pub reads: HashMap<String, HashSet<usize>>,
    pub writes: HashMap<String, HashSet<usize>>,
}
...
#[derive(Debug, Clone)]
pub struct FunctionInferenceMetric {
    pub total_ns: u128,
    pub runs: u64,
    pub last_ns: u128,
    pub ema_ns: u128,
...
impl Default for FunctionInferenceMetric {
    fn default() -> Self {
        Self {
            total_ns: 0,
            runs: 0,
...
pub static FUNCTION_METRICS: Lazy<Mutex<HashMap<usize, FunctionInferenceMetric>>> =
...
pub static EMA_ALPHA_RUNTIME: once_cell::sync::Lazy<std::sync::atomic::AtomicU64> =
    once_cell::sync::Lazy::new(|| {
        let v = std::env::var("AEONMI_EMA_ALPHA")
            .ok()
...
pub static WINDOW_CAP_RUNTIME: once_cell::sync::Lazy<std::sync::atomic::AtomicUsize> =
    once_cell::sync::Lazy::new(|| {
        let v = std::env::var("AEONMI_METRICS_WINDOW")
            .ok()
...
static SESSION_START_EPOCH_MS: once_cell::sync::Lazy<u64> =
    once_cell::sync::Lazy::new(|| current_epoch_ms());
...
pub fn session_start_epoch_ms() -> u64 {
    *SESSION_START_EPOCH_MS
}
pub fn set_ema_alpha(pct: u64) {
    if (1..=100).contains(&pct) {
        EMA_ALPHA_RUNTIME.store(pct, Ordering::Relaxed);
    }
}
pub fn set_window_capacity(n: usize) {
    if n >= 4 && n <= 256 {
        WINDOW_CAP_RUNTIME.store(n, Ordering::Relaxed);
    }
}

// Partial vs estimated full inference savings metrics with recent window history
#[derive(Debug, Clone)]
pub struct SavingsSample {
    pub partial_ns: u128,
    pub estimated_full_ns: u128,
    pub savings_ns: u128,
}
#[derive(Debug, Clone)]
pub struct SavingsMetrics {
    pub cumulative_savings_ns: u128,
    pub cumulative_partial_ns: u128,
    pub cumulative_estimated_full_ns: u128,
    pub history: VecDeque<SavingsSample>,
    pub window_partial_ns: u128,
    pub window_est_full_ns: u128,
    pub history_cap: usize,
}
impl Default for SavingsMetrics {
    fn default() -> Self {
        Self {
            cumulative_savings_ns: 0,
            cumulative_partial_ns: 0,
            cumulative_estimated_full_ns: 0,
            history: VecDeque::new(),
            window_partial_ns: 0,
            window_est_full_ns: 0,
            history_cap: 32,
        }
    }
}
impl SavingsMetrics {
    pub fn push_sample(&mut self, partial: u128, est_full: u128) {
        let savings = est_full.saturating_sub(partial);
        self.cumulative_partial_ns += partial;
        self.cumulative_estimated_full_ns += est_full;
        self.cumulative_savings_ns += savings;
        self.window_partial_ns += partial;
        self.window_est_full_ns += est_full;
        let sample = SavingsSample {
            partial_ns: partial,
            estimated_full_ns: est_full,
            savings_ns: savings,
        };
        if self.history.len() == self.history_cap {
            if let Some(old) = self.history.pop_front() {
                // adjust window counters removing old sample
                self.window_partial_ns = self.window_partial_ns.saturating_sub(old.partial_ns);
                self.window_est_full_ns = self
                    .window_est_full_ns
...
pub static SAVINGS_METRICS: Lazy<Mutex<SavingsMetrics>> =
    Lazy::new(|| Mutex::new(SavingsMetrics::default()));
#[allow(dead_code)]
pub fn record_savings(partial_ns: u128, estimated_full_ns: u128) {
    if partial_ns == 0 || estimated_full_ns == 0 {
        return;
    }
    if let Ok(mut sm) = SAVINGS_METRICS.lock() {
        sm.push_sample(partial_ns, estimated_full_ns);
    }
}
...
pub fn set_history_cap(n: usize) {
    if n >= 8 && n <= 256 {
        if let Ok(mut sm) = SAVINGS_METRICS.lock() {
            sm.history_cap = n;
            while sm.history.len() > sm.history_cap {
                if let Some(old) = sm.history.pop_front() {
                    sm.window_partial_ns = sm.window_partial_ns.saturating_sub(old.partial_ns);
                    sm.window_est_full_ns =
                        sm.window_est_full_ns.saturating_sub(old.estimated_full_ns);
                }
            }
        }
    }
...
pub fn record_function_infer(idx: usize, dur: u128) {
    ...
    let entry = m
        .entry(idx)
        .or_insert_with(FunctionInferenceMetric::default);
    entry.total_ns += dur;
    entry.runs += 1;
    entry.last_ns = dur;
    ...
    if entry.runs == 1 {
        ...
        entry.ema_ns = (entry.ema_ns * (100 - alpha) + dur * alpha) / 100;
    }
    let cap = WINDOW_CAP_RUNTIME.load(Ordering::Relaxed);
    if entry.window.len() == cap {
        entry.window.pop_front();
    }
    entry.window.push_back(dur);
}

pub fn set_deep_propagation(v: bool) {
    DEEP_PROPAGATION.store(v, Ordering::Relaxed);
}
pub fn get_deep_propagation() -> bool {
    DEEP_PROPAGATION.load(Ordering::Relaxed)
}
...
fn metrics_file_path() -> std::path::PathBuf {
    ...
    if let Err(e) = std::fs::create_dir_all(&base) {
        eprintln!("metrics dir create error: {e}");
    }
...
pub fn build_metrics_json() -> serde_json::Value {
    ...
    let function_metrics: HashMap<String, serde_json::Value> = fm
        .iter()
        .filter_map(|(idx, fm)| {
            if fm.last_run_epoch_ms > 0 && fm.last_run_epoch_ms < session_start {
                pruned += 1;
                return None;
            }
            let window_avg_ns = if !fm.window.is_empty() {
                fm.window.iter().copied().sum::<u128>() / fm.window.len() as u128
            } else {
                0
            };
            Some((
        ...
        .collect();
    let savings_pct = if sm.cumulative_estimated_full_ns > 0 {
        (sm.cumulative_savings_ns as f64 / sm.cumulative_estimated_full_ns as f64) * 100.0
    } else {
        0.0
    };
    let partial_pct = if sm.cumulative_estimated_full_ns > 0 {
        (sm.cumulative_partial_ns as f64 / sm.cumulative_estimated_full_ns as f64) * 100.0
    } else {
        0.0
    };
    let recent_window_savings_pct = if sm.window_est_full_ns > 0 {
        (sm.history.iter().map(|s| s.savings_ns).sum::<u128>() as f64
            / sm.window_est_full_ns as f64)
            * 100.0
    } else {
        0.0
    };
...
pub fn persist_metrics() {
    // Debounce check
    {
        if let Ok(mut last) = LAST_PERSIST.lock() {
            ...
            if let Some(prev) = *last {
                if now.duration_since(prev) < PERSIST_DEBOUNCE {
                    return;
                }
            }
            *last = Some(now);
    ...
    if CALL_GRAPH_METRICS.lock().is_ok() {
        // cheap check; build JSON anyway
        let json = build_metrics_json();
        #[cfg(test)]
        eprintln!(
            "persist_metrics json: {}",
...
pub fn install_shutdown_flush_guard() -> MetricsFlushGuard {
    MetricsFlushGuard
}
    ...
    if let Ok(data) = std::fs::read_to_string(path) {
        ...
        if let Ok(val) = serde_json::from_str::<serde_json::Value>(&data) {
                ...
                if let Some(obj) = fm.as_object() {
                    for (k, v) in obj {
                        if let Ok(idx) = k.parse::<usize>() {
                            let mut metric = FunctionInferenceMetric::default();
                            metric.runs = v.get("runs").and_then(|x| x.as_u64()).unwrap_or(0);
                            metric.total_ns =
                            ...
                            metric.ema_ns = v
                                .get("ema_ns")
                                .and_then(|x| x.as_u64())
                                .unwrap_or(metric.last_ns as u64)
                                as u128;
                            map.insert(idx, metric);
                        }
                    }
                }
            }
            if let Some(dp) = val.get("deepPropagation") {
                if let Some(b) = dp.as_bool() {
                    set_deep_propagation(b);
                }
            }
            if let Some(sv) = val.get("savings") {
                ...
                sm.cumulative_savings_ns = sv
                    .get("cumulative_savings_ns")
                    .and_then(|x| x.as_u64())
                    .unwrap_or(0) as u128;
                sm.cumulative_partial_ns = sv
                    .get("cumulative_partial_ns")
                    .and_then(|x| x.as_u64())
                    .unwrap_or(0) as u128;
                sm.cumulative_estimated_full_ns = sv
                    .get("cumulative_estimated_full_ns")
                    .and_then(|x| x.as_u64())
                    .unwrap_or(0) as u128;
                if let Some(arr) = sv.get("recent_samples").and_then(|x| x.as_array()) {
                    for s in arr {
                        let p = s.get("partial_ns").and_then(|x| x.as_u64()).unwrap_or(0) as u128;
                        let e = s
                            .get("estimated_full_ns")
                            .and_then(|x| x.as_u64())
                            .unwrap_or(0) as u128;
                        if p > 0 && e > 0 {
                            sm.push_sample(p, e);
...
#[allow(dead_code)]
pub fn reset_metrics_session() {
    if let Ok(mut m) = CALL_GRAPH_METRICS.lock() {
        m.reinfer_events = 0;
    }
}
...
pub fn reset_runtime_metrics_config() {
    set_ema_alpha(20);
    set_window_capacity(16);
    set_history_cap(32);
}
...
#[allow(dead_code)]
pub fn snapshot_call_graph_metrics() -> CallGraphMetrics {
    CALL_GRAPH_METRICS.lock().unwrap().clone()
}
...
pub fn compute_var_deps(ast: &ASTNode) -> VarDeps {
    ...
    fn expr_contains_identifier(n: &N) -> bool {
        match n {
            ...
            N::UnaryExpr { expr, .. } | N::Return(expr) | N::Log(expr) => {
            ...
            N::ProbabilityBranch {
                ...
                expr_contains_identifier(condition)
                    ...
                    || else_branch
                        .as_ref()
                        .map(|b| expr_contains_identifier(b.as_ref()))
                        .unwrap_or(false)
            }
            N::QuantumLoop {
                condition, body, ..
            ...
            N::For {
                ...
                init.as_ref()
                    .map(|expr| expr_contains_identifier(expr.as_ref()))
                    .unwrap_or(false)
                    || condition
                        .as_ref()
                        .map(|expr| expr_contains_identifier(expr.as_ref()))
            ...
            N::QuantumTryCatch {
                ...
                attempt_body.iter().any(expr_contains_identifier)
                    || catch_body
                        .as_ref()
                        .map(|b| b.iter().any(expr_contains_identifier))
                        .unwrap_or(false)
                    || success_body
                        .as_ref()
                        .map(|b| b.iter().any(expr_contains_identifier))
    ...
    fn walk(
        idx: usize,
        n: &N,
        reads: &mut HashMap<String, HashSet<usize>>,
        ...
        match n {
            ...
            N::If {
                ...
                walk(idx, condition, reads, writes);
                walk(idx, then_branch, reads, writes);
            ...
            N::For {
                ...
                if let Some(i) = init {
                    walk(idx, i, reads, writes);
                }
                if let Some(c) = condition {
                    walk(idx, c, reads, writes);
                }
                if let Some(inc) = increment {
                    walk(idx, inc, reads, writes);
                }
                walk(idx, body, reads, writes);
            }
            ...
            N::ArrayLiteral(elements) => {
                for el in elements {
                    walk(idx, el, reads, writes);
                }
            }
            N::IndexExpr { array, index } => {
                walk(idx, array, reads, writes);
                walk(idx, index, reads, writes);
            }
            N::UnaryExpr { expr, .. } => {
                walk(idx, expr, reads, writes);
            }
            N::Call { callee, args } => {
                walk(idx, callee, reads, writes);
                for a in args {
    ...
    if let ASTNode::Program(items) = ast {
        for (idx, node) in items.iter().enumerate() {
            if let N::Function { body, .. } = node {
                for c in body {
                    walk(idx, c, &mut var_reads, &mut var_writes);
                }
            }
        }
    }
...
pub fn parse_or_cached(source: &str) -> Result<ASTNode, String> {
    ...
    match parser.parse() {
        Ok(ast) => {
            let spans = index_top_level(&ast, source);
            *CACHE.lock().unwrap() = Some(CachedParse {
                hash,
                ast: ast.clone(),
        ...
        Err(ParserError {
            ...
            line,
            column,
        }) => Err(format!("{message} at {line}:{column}")),
    }
}
...
pub fn parse_or_partial(source: &str) -> Result<(ASTNode, bool), String> {
    ...
    if !dirty.changed {
        return Ok((prev.ast.clone(), false));
    }
    // Count overlapping nodes
    let mut overlap_indices: Vec<usize> = Vec::new();
    for (i, sp) in prev.top_spans.iter().enumerate() {
        if overlaps(
            dirty.first_changed_line,
            dirty.last_changed_line,
            sp.start_line,
            sp.end_line,
        ) {
    ...
    if !overlap_indices.is_empty()
        && overlap_indices.windows(2).all(|w| w[1] == w[0] + 1)
        && overlap_indices.len() <= 8
    {
        ...
        let first = overlap_indices[0];
        let last = *overlap_indices.last().unwrap();
        let sp = &prev.top_spans[first];
        let end_span = &prev.top_spans[last];
        let lines: Vec<&str> = source.lines().collect();
        let fragment_src =
            lines[(sp.start_line.saturating_sub(1))..end_span.end_line.min(lines.len())].join("\n");
                ...
                if let ASTNode::Program(new_items) = new_ast.clone() {
                    if let ASTNode::Program(mut old_items) = prev.ast.clone() {
                        ...
                        if new_items.len() == overlap_indices.len() {
                            for (offset, idx) in overlap_indices.iter().enumerate() {
                                if *idx < old_items.len() {
                                    old_items[*idx] = new_items[offset].clone();
                                }
                            }
                            let updated = ASTNode::Program(old_items);
                            let spans = index_top_level(&updated, source);
                            *CACHE.lock().unwrap() = Some(CachedParse {
                                hash: String::new(),
                                ast: updated.clone(),
                                source: source.to_string(),
                            ...
                            *LAST_REPLACED_INDEX.lock().unwrap() = Some(first);
    ...
    if overlap_indices.is_empty() {
        ...
        let ast = parse_or_cached(source)?;
        return Ok((ast, true));
    }
...
fn index_top_level(ast: &ASTNode, source: &str) -> Vec<NodeSpan> {
    ...
    if let ASTNode::Program(items) = ast {
        ...
        for node in items {
            starts.push(node_start_line(node));
        }
...
fn node_start_line(node: &ASTNode) -> usize {
    match node {
        ASTNode::Function { line, .. } => *line,
        ASTNode::VariableDecl { line, .. } => *line,
...
fn compute_dirty_info(new_src: &str) -> DirtyInfo {
    ...
    if let Some(prev) = cache.as_ref() {
        ...
        let mut last_old = old_lines.len().saturating_sub(1);
        let mut last_new = new_lines.len().saturating_sub(1);
        ...
        while last_old > first && last_new > first && old_lines[last_old] == new_lines[last_new] {
            last_old -= 1;
            last_new -= 1;
        }
        DirtyInfo {
            changed: true,
            first_changed_line: first + 1,
            last_changed_line: (last_new + 1).max(first + 1),
        }
    } else {
        DirtyInfo {
            changed: true,
            first_changed_line: 1,
            last_changed_line: new_src.lines().count(),
        }
    }
}

#[allow(dead_code)]
pub fn dirty_region(source: &str) -> DirtyInfo {
...
pub fn debug_snapshot() -> serde_json::Value {
    ...
    serde_json::json!({ "functions": fm.iter().map(|(i,m)| (i.to_string(), serde_json::json!({"runs": m.runs, "last_ns": m.last_ns, "ema_ns": m.ema_ns, "window": m.window, "last_run_epoch_ms": m.last_run_epoch_ms}))).collect::<serde_json::Value>(), "savings_history_len": sm.history.len(), "savings_recent_samples": sm.history.iter().map(|s| { serde_json::json!({"partial": s.partial_ns, "est_full": s.estimated_full_ns, "savings": s.savings_ns}) }).collect::<Vec<_>>() })
}
