...
static ARTIFACT_CACHE: Lazy<Mutex<HashMap<String, ArtifactEntry>>> =
    Lazy::new(|| Mutex::new(HashMap::new()));
const MAX_ENTRIES: usize = 128;
const MAX_TOTAL_BYTES: usize = 10 * 1024 * 1024; // 10MB
static LOGGING_ENABLED: AtomicBool = AtomicBool::new(true);

pub fn get_artifact(hash: &str) -> Option<ArtifactEntry> {
    let mut g = ARTIFACT_CACHE.lock().ok()?;
    if let Some(e) = g.get_mut(hash) {
...
fn prune_locked(m: &mut HashMap<String, ArtifactEntry>) {
    ...
    if m.len() > MAX_ENTRIES || total_bytes(m) > MAX_TOTAL_BYTES {
        let before_entries = m.len();
        let before_bytes = total_bytes(m);
        ...
        for e in entries {
            if m.len() <= MAX_ENTRIES && bytes <= MAX_TOTAL_BYTES {
                break;
            }
            if m.remove(&e.hash).is_some() {
                bytes = total_bytes(m);
            }
        }
        if LOGGING_ENABLED.load(Ordering::Relaxed) {
            eprintln!(
                "[artifact_cache] pruned: entries {}->{} bytes {}->{}",
                before_entries,
                m.len(),
                before_bytes,
                total_bytes(m)
...
#[allow(dead_code)]
pub fn set_cache_logging(enabled: bool) {
    LOGGING_ENABLED.store(enabled, Ordering::Relaxed);
}

#[allow(dead_code)]
pub fn cache_stats() -> (usize, usize) {
    if let Ok(m) = ARTIFACT_CACHE.lock() {
        (m.len(), total_bytes(&m))
    } else {
        (0, 0)
    }
...
fn total_bytes(m: &HashMap<String, ArtifactEntry>) -> usize {
    m.values().map(|e| e.size).sum()
}

#[allow(dead_code)]
pub fn clear_artifacts() {
    let _ = ARTIFACT_CACHE.lock().map(|mut m| m.clear());
}
