...
fn savings_history_reload_preserves_window_counters() {
    ...
    assert_eq!(dump_before_code, 0, "metrics-dump failed: {}", sanitize_output(&out_before));
    ...
    let savings_before = v_before.get("savings").and_then(|v| v.as_object()).expect("savings before");
    let before_partial = savings_before.get("recent_window_partial_ns").and_then(|v| v.as_u64()).unwrap_or(0);
    let before_estimated = savings_before.get("recent_window_estimated_full_ns").and_then(|v| v.as_u64()).unwrap_or(0);

    // New invocation reloads from disk; ensure counters survive round-trip.
    let (dump_after_code,out_after)= run(&["metrics-dump"]);
    assert_eq!(dump_after_code, 0, "metrics-dump reload failed: {}", sanitize_output(&out_after));
    ...
    let savings_after = v_after.get("savings").and_then(|v| v.as_object()).expect("savings after");
    let after_partial = savings_after.get("recent_window_partial_ns").and_then(|v| v.as_u64()).unwrap_or(0);
    let after_estimated = savings_after.get("recent_window_estimated_full_ns").and_then(|v| v.as_u64()).unwrap_or(0);

    assert_eq!(after_partial, before_partial, "recent partial window counter changed after reload");
    assert_eq!(after_estimated, before_estimated, "recent estimated window counter changed after reload");
