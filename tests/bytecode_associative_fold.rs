...
#[test]
fn fold_bool_and_chain() {
    let v = eval("return true && true && false && true; ");
    match v {
...
#[test]
fn fold_bool_or_chain() {
    let v = eval("return false || false || true || false; ");
    match v {
        Some(Value::Bool(b)) => assert!(b),
        _ => panic!("bad {v:?}"),
    }
