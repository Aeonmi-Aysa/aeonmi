...
use aeonmi_project::core::quantum_extract::{circuit_to_pseudo_qasm, extract_circuit};

#[test]
fn pseudo_qasm_contains_qreg() {
    let src = "superpose(q0); entangle(q0, q1);"; // sample circuit exercising multi-qubit op
    ...
    assert!(qasm.contains("superpose q[0];"));
    assert!(qasm.contains("entangle q[0],q[1];"));
}
