use aeonmi_project::core::{diagnostics::*, lexer::Lexer, parser::Parser};
...
fn test_quantum_native_syntax_lexing() {
    ...
    assert!(token_kinds.iter().any(|t| matches!(
        t,
        aeonmi_project::core::token::TokenKind::QuantumBracketOpen
    )));
    assert!(token_kinds
        .iter()
        .any(|t| matches!(t, aeonmi_project::core::token::TokenKind::QuantumIn)));
    assert!(token_kinds
        .iter()
        .any(|t| matches!(t, aeonmi_project::core::token::TokenKind::QuantumBind)));
}
...
fn test_quantum_native_syntax_parsing() {
    ...
    let ast = parser
        .parse()
        .expect("Should parse quantum variable declaration");
...
fn test_quantum_function_syntax() {
    ...
    match ast {
        ...
        Err(e) => println!(
            "Parse error (expected for incomplete implementation): {}",
            e
        ),
    }
}
